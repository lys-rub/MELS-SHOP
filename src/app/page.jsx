      ...
      <div className="mx-auto max-w-7xl px-6 pb-20 lg:px-8">
        <div className="rounded-[2rem] border border-white/80 bg-white/85 p-8 shadow-soft sm:p-10">
          <div className="grid gap-10 lg:grid-cols-3 lg:items-center">
            <div>
              <p className="text-sm uppercase tracking-[0.25em] text-brand-500">Sélection premium</p>
            ...
            <div className="grid gap-5 sm:grid-cols-2 lg:col-span-2">
              {featuredProducts.map((item) => (
                <article key={item.name} className="rounded-[1.8rem] border border-slate-200 bg-slate-50/70 p-6">
                  ...
                  <p className="mt-5 text-xl font-semibold text-brand-700">{formatPrice(item.price)}</p>
                </article>
              ))}
            </div>
          </div>
