    ...
    <article className="group relative rounded-[2rem] border border-brand-100 bg-white/90 p-6 shadow-soft transition hover:-translate-y-1 hover:border-brand-300">
      <div className={`relative aspect-square w-full rounded-[1.8rem] ${product.accent} p-6`}>
        <div className="flex h-full items-end justify-between">
          <div>
            <p className="text-xs uppercase tracking-[0.28em] text-gold-700">{product.tag}</p>
          </div>
        ...
        <button
          ...
          aria-pressed={fav}
          onClick={() => setFav((s) => !s)}
          className={`absolute right-3 top-3 inline-flex items-center justify-center rounded-full p-2 transition-shadow ${fav ? 'bg-gold-500 text-white shadow-lg' : 'bg-white/90 text-gold-700 border border-slate-200'}`}
        >
      ...
      <div className="mt-6 space-y-4">
        ...
        <div className="flex items-center justify-between gap-4">
          <p className="text-xl font-semibold text-gold-700">{formatPrice(product.price)}</p>
          <Link href={`/shop/${product.id}`} className="rounded-full border border-gold-200 bg-gold-50 px-4 py-2 text-sm font-semibold text-gold-700 transition hover:bg-gold-100 shadow-sm">
            Voir plus
          </Link>
        </div>
      </div>
