          ...
          <nav className="hidden items-center gap-8 lg:flex">
            {navItems.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className={`text-sm ${item.bold ? 'font-bold' : 'font-medium'} text-gold-700 transition hover:text-gold-900`}
              >
                {item.label}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-2">
            {actions.map((a) => (
              <Link
                ...
                href={a.href}
                aria-label={a.aria}
                className="inline-flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-full border border-gold-100 bg-white text-gold-700 transition hover:border-gold-300 hover:text-gold-900"
              >
