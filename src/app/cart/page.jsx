  ...
  return (
    <section className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(200,79,66,0.08),_transparent_35%),_linear-gradient(180deg,_rgba(255,250,246,1)_0%,_rgba(246,239,232,1)_100%)] py-20">
      <div className="mx-auto max-w-6xl px-6 lg:px-8">
        <div className="mb-12">
          <p className="text-sm uppercase tracking-[0.3em] text-brand-500">Panier</p>
        ...
        <div className="grid gap-8 xl:grid-cols-[1.5fr_0.9fr]">
            ...
            {cartItems.map((item) => (
              <div key={item.id} className="rounded-[2rem] border border-slate-200 bg-white/90 p-6 shadow-soft">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <p className="text-sm uppercase tracking-[0.28em] text-brand-500">{item.tag}</p>
                  ...
                  <div className="space-y-2 text-right">
                    <p className="text-xl font-semibold text-brand-700">{formatPrice(item.price)}</p>
          ...
          <aside className="rounded-[2rem] border border-slate-200 bg-white/90 p-8 shadow-soft">
            ...
            <Link href="/checkout" className="mt-8 inline-flex w-full items-center justify-center rounded-full bg-brand-600 px-6 py-4 text-sm font-semibold text-white transition hover:bg-brand-700">
              Procéder au paiement
            </Link>
            <Link href="/shop" className="mt-4 inline-flex w-full items-center justify-center rounded-full border border-brand-200 bg-white px-6 py-4 text-sm font-semibold text-brand-700 transition hover:bg-brand-50">
